...
class TrendCenterApp(ctk.CTk):
    ...
    def show_login(self):
        ...
        brand_head = ctk.CTkFrame(brand_panel, fg_color=None)
        brand_head.pack(fill="x", padx=34, pady=(34, 10))
        try:
            logo_p = resource_path("icon.png")
            if os.path.exists(logo_p):
                logo_img = ctk.CTkImage(light_image=Image.open(logo_p), size=(76, 76))
                ctk.CTkLabel(brand_head, image=logo_img, text="").pack(side="right", padx=(32, 0))
        except Exception:
            pass
        title_box = ctk.CTkFrame(brand_head, fg_color=None)
        title_box.pack(side="right", fill="x", expand=True)
        ctk.CTkLabel(title_box, text=fix_arabic(SHOP_NAME, for_ui=True), font=("Arial", 26, "bold"), text_color=COLOR_WHITE, anchor="e").pack(fill="x", pady=(2, 4))
        ctk.CTkLabel(title_box, text=fix_arabic("كل جهاز ... لها قصتها", for_ui=True), font=FONT_BOLD, text_color=COLOR_WHITE, anchor="e").pack(fill="x", pady=(2, 0))
        ...
        category_data = [
            ("sales.png", "إكسسوارات الهواتف", "شواحن • كيابل • حماية"),
            ("maintenance.png", "البلايستيشن والألعاب", "ملحقات • أيدي • خدمات"),
            ("inventory.png", "الكمبيوتر والشبكات", "قطع • تجهيزات • توصيل"),
            ("settings.png", "أنظمة المراقبة", "كاميرات • تسجيل • تركيب"),
        ]
        for idx, (icon_name, label, detail) in enumerate(category_data):
            ...
            tile = ctk.CTkFrame(categories, fg_color=None, corner_radius=14, border_width=1, border_color=tile_border)
            ...
            try:
                ...
                if os.path.exists(icon_path):
                    category_img = ctk.CTkImage(light_image=Image.open(icon_path), size=(38, 38))
                    ctk.CTkLabel(tile, image=category_img, text="").pack(side="right", padx=(12, 8), pady=16)
            except Exception:
                pass
            text_box = ctk.CTkFrame(tile, fg_color=None)
            text_box.pack(side="right", fill="both", expand=True, padx=(0, 10), pady=12)
    ...
    def show_dashboard(self):
        ...
        try:
            ...
            pass
        ctk.CTkLabel(brand, text=fix_arabic(s_name, for_ui=True), font=FONT_BOLD, text_color=COLOR_WHITE, wraplength=260).pack(pady=(0, 3))
        ...
        if self.current_role == "employee":
            ...
            add_nav_group("الصيانة وخدمة العملاء", [("تسعيرة قطع الصيانة", self.ui_maintenance_parts), ("سجل استلام وتسليم الأجهزة", self.ui_service_register), ("استعلام نقاط الولاء", self.ui_loyalty)], COLOR_TEAL)
        else:
    ...
    def _service_register_contract(self, order, kind):
        ...
        rtl("ترند سنتر الأردن", title_font, 26, COLOR_WHITE)
    ...
    def generate_invoice(self, total, type="SALE", extra=None):
        ...
        d.text((250, 50), fix_arabic(s_name, for_ui=False), fill=(255,255,255), font=bfont, anchor="mm")
